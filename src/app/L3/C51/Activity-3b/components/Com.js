...
export default function Com() {
  ...
  const handleDateChange = (index, value, inputRef) => {
    if (value.length <= 1 && /^\d*$/.test(value)) {
      ...
      if (value && index < date.length - 1) {
        const nextInput = inputRef.current.parentElement.children[index + 1];
        if (nextInput) {
          nextInput.focus();
        }
      }
  ...
  const handleDateKeyDown = (index, e) => {
    ...
    if (e.key === 'Backspace' && !date[index] && index > 0) {
      e.preventDefault();
      const prevInput = dateInputRefs.current[index - 1];
      if (prevInput) {
        prevInput.focus();
          ...
          <div className="flex items-center gap-1.5 ml-auto">
            {date.map((digit, index) => (
              <input
                ...
                maxLength="1"
                value={digit}
                onChange={(e) => handleDateChange(index, e.target.value, { current: dateInputRefs.current[index] })}
        ...
        <main className="flex-1 p-6 pb-6 flex flex-col gap-4">
          <div className="flex items-center gap-3">
            ...
            <div className="relative ml-auto">
              ...
              <div className={`font-bold text-sm px-3 py-1.5 rounded-full border ${
                chequeMode === "acpayee" 
