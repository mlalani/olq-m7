...
export default function Com() {
  ...
  const handleDownloadPDF = () => {
    ...
    doc.text(`Shopping Budget: ${shoppingBudget || "-"} ${visitingCurrency}`, 10, 94);
...
<br />
      ...
      <section className="mx-auto max-w-5xl px-4 pb-4">
        <div className="rounded-2xl bg-slate-800 p-6 shadow border border-slate-900">
          <h2 className="text-xl font-semibold">Research Cost of Shopping</h2>
          <p className="mt-1 ">
            Input item which you want to buy and compare the price in{" "}
            <b>{homeCurrency}</b> vs in <b>{visitingCurrency}</b> online.
          </p>

          <div className="mt-3 grid gap-4 sm:grid-cols-3">
            <div>
              <label className="block text-md font-medium mb-[10px]">Item</label>
              <input
                className="mt-1 w-full rounded-xl border px-3 py-2"
                placeholder="e.g., hoodie, toy, magnet"
                value={shoppingItem}
            ...
            <div>
              <label className="block text-md font-medium mb-[10px]">
                Price at home ({homeCurrency})
              </label>
              <input
                type="number"
                step="any"
                className="mt-1 w-full rounded-xl border px-3 py-2"
                placeholder={`e.g., 120 (${homeCurrency})`}
                value={shoppingHomePrice}
                onChange={(e) => setShoppingHomePrice(e.target.value)}
                onWheel={e => e.target.blur()}
              />
            </div>
            <div>
              <label className="block text-md font-medium mb-[10px]">
                Price there ({visitingCurrency})
              </label>
              <input
                type="number"
                step="any"
                className="mt-1 w-full rounded-xl border px-3 py-2"
                placeholder={`e.g., 20 (${visitingCurrency})`}
